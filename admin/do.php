...
function associateNetIDs($ids) {
	...
	foreach ($rows as $record) {
		...
		$db->exec($sql_udpate_students) or die(mysql_error());

		$sql_udpate_account = "update accounts set used=1 where id=$record_id";
		$db->exec($sql_udpate_account) or die(mysql_error());
...
function associateNetID($userid) {
	...
	$db->exec($update_account) or die(mysql_error());
...
Administrator
$host_upper______________________________________________________THISISANAUTOMATEDRESPONSE.
